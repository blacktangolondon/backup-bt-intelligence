...
function openFilterSelector() {
  ...
  function renderInputs() {
    ...
    if (nameSel.value === 'Asset Class') {
      ...
      inpDiv.appendChild(chk);
      const lbl = document.createElement('label'); lbl.textContent = 'True';
      inpDiv.appendChild(lbl);
    } else {
      const op = document.createElement('select');
      ['>=','<='].forEach(sym => {
        const x = document.createElement('option'); x.value = sym; x.textContent = sym;
        op.appendChild(x);
      });
  ...
  div.querySelector('.apply-filter-btn').addEventListener('click', () => {
    ...
    if (f.filterName === 'Asset Class') {
      ...
      f.value = inpDiv.querySelector('input[type="checkbox"]').checked;
    } else {
...
function updatePortfolioSteps() {
  ...
  portfolioFilters.forEach((f, i) => {
    ...
    let text;
    if (f.filterName === 'Asset Class') text = `${f.filterName}: ${f.value}`;
    else if (priceKeyAreaFilters.includes(f.filterName)) text = f.filterName;
    else text = `${f.filterName} ${f.operator} ${f.value}`;
...
function generatePortfolioNew() {
  ...
  for (const instrument in dataObj) {
    ...
    for (let i = 1; i < portfolioFilters.length; i++) {
      ...
      if (priceKeyAreaFilters.includes(f.filterName)) {
        const arr = window.pricesData[priceKey][instrument] || [];
        const price = arr[arr.length - 1];
        const summaryStr = info.summaryLeft[4] || '';
        const [ka1, ka2] = summaryStr.split('/').map(s => parseFloat(s.trim()));
        let ok;
        switch (f.filterName) {
  ...
  const averages = portfolioFilters.slice(1).map(f => {
  ...
  const summaryDiv = document.createElement('div'); summaryDiv.id = 'portfolio-summary';
  let summaryHtml = `
