...
// ───────────────────────────────────────────────────────────
// Block 4: Fundamentals (Earnings/Dividend/Valuation)
// ───────────────────────────────────────────────────────────
export function updateBlock4(instrumentName, groupData){
  ...
  // raw fields
  const eps       = num(info.eps);            // Earnings Per Share
  const pe        = num(info.pe_ratio);       // Price / Earnings
  const pb        = num(info.pb_ratio);       // Price / Book
  const dyPct     = num(info.div_yield);      // Dividend Yield (già espresso in %, nei dati)
  const payout    = num(info.payout_ratio);   // frazione (0–1)

  // derived
  const dpsEst    = (eps != null && payout != null) ? eps * payout : null;                // DPS stimato
  const cover     = (payout != null && payout > 0) ? (1 / payout) : null;                 // Dividend Cover
  const earnYield = (pe != null && pe > 0) ? (1 / pe) : null;                             // Earnings Yield
  ...
  block4.innerHTML = `
    <div class="fund-card">
      ...
      <div class="fund-grid">
        ${renderFundRow("EPS", eps)}
        ${renderFundRow("Earnings Yield", earnYield, "%", true)}   <!-- 1/PE in % -->
      </div>

      <div class="fund-subtitle">Dividend focus</div>
      <div class="fund-grid">
        ${renderFundRow("Dividend Yield", dyPct, "%")}             <!-- già percentuale -->
        ${renderFundRow("Dividend Cover", cover)}                   <!-- = 1 / payout -->
        ${renderFundRow("DPS (est.)", dpsEst)}                      <!-- payout × EPS -->
      </div>

      <div class="fund-subtitle">Valuation focus</div>
      <div class="fund-grid">
        ${renderFundRow("P/E", pe)}
        ${renderFundRow("P/B", pb)}
      </div>
...
// label/value cell with optional % rendering
function renderFundRow(label, value, suffix="", isFraction=false){
  let out = "–";
  if (value != null && value !== "") {
    if (typeof value === "number") {
      // if isFraction==true we convert 0.1234 -> 12.34%
      if (isFraction) out = (value * 100).toFixed(2) + (suffix || "");
      else out = Number.isFinite(value) ? String(value) + (suffix || "") : "–";
    } else {
      out = String(value);
    }
  }
  return `<div class="fund-row"><span>${label}</span><strong>${out}</strong></div>`;
}
