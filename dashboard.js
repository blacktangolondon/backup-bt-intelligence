...
function drawMostCorrelatedChart(top10) {
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      plugins: { legend: { display: false }, title: { display: true, text: '10 MOST CORRELATED INSTRUMENTS', color: 'white', font: { size: 14, family: 'Arial' } } }
    }
  });
}
...
export function initBlock3Tabs() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      document.querySelector(`.portfolio-tab-content[data-category="${target}"]`)?.classList.add("active");
    });
  });
}
